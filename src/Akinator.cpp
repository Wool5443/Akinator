...
ErrorCode _compare(Tree* dataTree)
{
    ...
    {
        PRINT_SAY("%s and %s are similar because they", obj1, obj2);

        while (!obj1Pop.error && !obj2Pop.error && obj1Pop.value == obj2Pop.value)
        {
            FOLLOW_PATH(obj1Pop.value, 
            {
                PRINT_SAY("are %s", curNode->value);
                curNode = curNode->left;
            },
            {
                PRINT_SAY("are not %s", curNode->value);
                          ...
                          StackElementResult prevPop)
{
    ...
    if (!objPop.error)
    {
        ...
        while (!objPop.error)
        {
            ...
            {
                PRINT_SAY("is %s", curNode->value); 
                curNode = curNode->left;
            },
            {
                PRINT_SAY("is not %s", curNode->value);
                curNode = curNode->right;
            });
            objPop = Pop(stack);
        }
